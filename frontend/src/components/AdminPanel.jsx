...
export default function AdminPanel() {
  ...
  // MODIFICADO: Agregamos imagen_url al estado inicial
    ...
    try {
      const response = await fetch(url, {
        ...
        body: JSON.stringify({
          ...
          imagen_url: formData.imagen_url, // MODIFICADO: Enviamos la URL al backend
        }),
      });
  ...
  const handleEdit = (producto) => {
    ...
    setFormData({
      ...
      imagen_url: producto.imagen_url || "", // MODIFICADO: Cargamos la URL si existe
    });
    setIsEditing(true);
  };
  ...
  return (
    <div className="mt-8 bg-gray-50 p-6 rounded-lg border">
      ...
      {/* MODIFICADO: sm:grid-cols-5 para que quepa el nuevo input */}
      <form
        onSubmit={handleSubmit}
        className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 mb-8 bg-white p-4 shadow-sm"
      >
        <input
          type="text"
          name="nombre"
        ...
        <button
          type="submit"
          className="bg-black text-white font-bold p-2 hover:bg-gray-800 transition"
        >
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-left bg-white border">
          <thead className="bg-black text-white uppercase text-sm">
            <tr>
              <th className="p-3">Img</th> {/* NUEVA COLUMNA */}
          ...
          <tbody>
            {productos.map((p) => (
              <tr key={p.id_producto} className="border-b hover:bg-gray-50">
                {/* NUEVA CELDA: Miniatura de la imagen */}
                <td className="p-3">
                  <img
                    src={
                      p.imagen_url
                        ? p.imagen_url
                        : `https://ui-avatars.com/api/?name=${p.nombre}&background=random`
                    }
                    alt={p.nombre}
